...
const Subject = ({ open, toogleDrawer, account, deleteAccount, match, location, history }) => {
  ...
  const getImages = imageArray => {
    imageArray.forEach(async image => {
      const response = await fetch(imagesURL + branch + '/' + category.toLowerCase() + '/' + subjectName + '/' + image.filename, {
        method: 'GET',
        credentials: 'include',
        headers: {
    ...
    <div className={classes.root}>
      ...
      <div className={classNames(classes.rest, !open && classes.closed)}>
        <Breadcrumbs>
          ...
          <Link component='button' variant='body1' onClick={() => { history.push(`/info/${branch}/${category}/${subject.name}`) }}>
            {titleCase(subject.name) ? titleCase(subject.name) : ''}
          </Link>
        </Breadcrumbs>
        <Typography variant='h2' align='center' color='textPrimary'>{subject.name}</Typography>
        <Typography variant='h4' align='center' color='textPrimary'>{subject.text}</Typography>
